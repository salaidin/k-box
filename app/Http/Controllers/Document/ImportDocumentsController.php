        ...
        if ($request->wantsJson()) {
            ...
            try {
                if ($remote) {
                    ...
                    if (! is_dir($url)) {
                        return new JsonResponse(['folder_import' => trans('errors.import.folder_not_readable', ['folder' => $url])], 422);
                    }
                ...
                return new JsonResponse([''.(($remote) ? 'remote_import' : 'folder_import') => $aex->render($auth_user)], 422);
            } catch (\InvalidArgumentException $ex) {
                \Log::error('Import store', ['exception' => $ex]);

                return new JsonResponse([''.(($remote) ? 'remote_import' : 'folder_import') => $ex->getMessage()], 422);
            } catch (\Exception $ex) {
                \Log::error('Import store', ['exception' => $ex]);
